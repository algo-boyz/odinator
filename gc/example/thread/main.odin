...
worker_task :: proc(t: thread.Task) {
    ...
    // Create a dedicated memory arena for this specific job/task
    task_arena := gc.new_arena()
    // IMPORTANT: Ensure the arena is destroyed when the task finishes.
    // This will free all memory allocated within this arena using arena_alloc.
    defer gc.destroy_arena(task_arena)
    fmt.printf("[Worker %d / Job %d] Created arena %p.\n", worker_id, data.job_id, task_arena)

    alloc_size := (data.job_id % 5 + 1) * 256 // Allocate some data based on job_id
    work_buffer: rawptr
    work_buffer = gc.arena_alloc(task_arena, alloc_size)

    if work_buffer == nil {
        fmt.eprintf("[Worker %d / Job %d] Error: Failed to allocate %d bytes in arena %p.\n",
            worker_id, data.job_id, alloc_size, task_arena)
        // Can't proceed with work if allocation failed
    } else {
        fmt.printf("[Worker %d / Job %d] Allocated %d bytes at %p within arena %p.\n",
            worker_id, data.job_id, alloc_size, work_buffer, task_arena)
