// Enhanced Odin wrapper for libgc
// Updated: 2025-04-04
...
Config :: struct {
	incremental: bool,     // Enables incremental collection
	parallel: bool,        // Enables parallel collection
	log_level: Log_Level,  // Controls logging verbosity
...
Stats :: struct {
	...
	collection_time: int,   // Total time spent in collections (ms)
}
...
do_alloc :: proc(
    ...
    // alignment is irrelevant for free modes.
...
DEFAULT_CONFIG :: Config {
	...
	parallel = true,
	log_level = .Warn,
	initial_heap = 0,  // Use GC default
	max_heap = 0,      // Unlimited
}

// Initialize the GC with configuration options
initialize :: proc(config := DEFAULT_CONFIG) -> runtime.Context {
	...
	if config.incremental {
		enable_incremental()
		log(.Info, "Enabled incremental collection")
	}
...
get_stats :: proc() -> Stats {
	stats := Stats{
		...
		// Note: collection_count and collection_time would require additional tracking
	}
	return stats
}
...
// Simplified region allocator using GC underneath
Region :: struct {
...
// Create a new region
create_region :: proc() -> ^Region {
	region := new(Region, allocator)
	region.objects = make([dynamic]rawptr, allocator)
	return region
}

// Allocate memory in a region
region_alloc :: proc(region: ^Region, size: int) -> rawptr {
	sync.mutex_lock(&region.mutex)
	defer sync.mutex_unlock(&region.mutex)
	
	ptr := malloc(c.size_t(size))
	if ptr != nil {
		append(&region.objects, ptr)
	}
	return ptr
}

// Free all memory in a region
region_free_all :: proc(region: ^Region) {
	sync.mutex_lock(&region.mutex)
	defer sync.mutex_unlock(&region.mutex)
...
// Unit test for regions
@(test)
test_regions :: proc(t: ^testing.T) {
	ctx := initialize()
	context = ctx
	
	region := create_region()
	defer destroy_region(region)
	
	// Allocate some objects
	for i in 0..<100 {
		ptr := region_alloc(region, 1024)
		testing.expect(t, ptr != nil, "Region allocation failed")
	}
	
	// Free all at once
	region_free_all(region)
	testing.expect(t, len(region.objects) == 0, "Region free failed")
}