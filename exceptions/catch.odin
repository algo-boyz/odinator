...
reset :: proc(loc := #caller_location) {
    ...
    if err != 0 {
        ...
        // Clear err state before panicking
...
try_with_default :: proc(try_proc: proc() -> $T, default_value: T, loc := #caller_location) -> T {
    result: T = default_value
    success := false
    ...
    try(
        ...
        proc(e: Error) {
            // Reset to default value on exception
